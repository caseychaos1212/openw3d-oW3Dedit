...
static bool ReplaceFileAtomically(
    ...
    QString& errorMessage)
{
    ...
    if (!source.open(QIODevice::ReadOnly)) {
        errorMessage = QObject::tr("Failed to open staged file: %1")
    ...
    if (!destination.open(QIODevice::WriteOnly)) {
        errorMessage = QObject::tr("Failed to stage replacement: %1")
    ...
    while (true) {
        ...
        if (bytesRead < 0) {
            ...
            errorMessage = QObject::tr("Failed to read staged file: %1")
                .arg(source.errorString());
            return false;
        }
        if (bytesRead == 0) {
        ...
        while (totalWritten < bytesRead) {
            const qint64 bytesWritten = destination.write(
                buffer.constData() + totalWritten,
                bytesRead - totalWritten);
            if (bytesWritten <= 0) {
                destination.cancelWriting();
                errorMessage = QObject::tr("Failed to write staged replacement: %1")
                    .arg(destination.errorString());
                return false;
            }
    ...
    if (!destination.commit()) {
        errorMessage = QObject::tr("Failed to commit staged replacement: %1")
            .arg(destination.errorString());
        return false;
    }
...
void MainWindow::on_actionImportJsonBatch_triggered()
{
    ...
    // Recursively mirror the relative layout produced by batch JSON export.
    ...
    while (jsonIt.hasNext()) {
        jsonFiles.append(QFileInfo(jsonIt.next()));
    }
    std::sort(jsonFiles.begin(), jsonFiles.end(), [&](const QFileInfo& lhs, const QFileInfo& rhs) {
        const QString lhsRelative = SanitizeRelativePath(
            jsonDirObj.relativeFilePath(lhs.absoluteFilePath()));
        const QString rhsRelative = SanitizeRelativePath(
            jsonDirObj.relativeFilePath(rhs.absoluteFilePath()));
        return lhsRelative.compare(rhsRelative, Qt::CaseInsensitive) < 0;
        });

    if (jsonFiles.isEmpty()) {
        QMessageBox::information(this, tr("Import JSON Batch"),
            tr("No .json files found in %1 or its subdirectories.").arg(jsonDir));
        return;
    }

    // Match the JSON relative path generated by BuildBatchJsonRelativePath. A
    // key can contain multiple targets (for example foo.w3d and foo.wlt); those
    // matches are ambiguous and must never be selected implicitly.
    QDir w3dDirObj(w3dDir);
    std::map<QString, QStringList> w3dByJsonRelativePath;
    QDirIterator w3dIt(
        w3dDir,
        QStringList{ "*.w3d", "*.W3D", "*.wlt", "*.WLT" },
        QDir::Files | QDir::NoSymLinks,
        QDirIterator::Subdirectories);
    while (w3dIt.hasNext()) {
        const QString absolutePath = QDir::cleanPath(w3dIt.next());
        const QString relativePath = SanitizeRelativePath(
            w3dDirObj.relativeFilePath(absolutePath));
        const QString jsonRelativeKey = QDir::cleanPath(
            BuildBatchJsonRelativePath(relativePath)).toLower();
        w3dByJsonRelativePath[jsonRelativeKey].append(absolutePath);
    }
    for (auto& [key, targetPaths] : w3dByJsonRelativePath) {
        (void)key;
        targetPaths.sort(Qt::CaseInsensitive);
    }

    std::map<QString, int> jsonRelativePathCounts;
    for (const QFileInfo& jsonInfo : jsonFiles) {
        const QString relativePath = SanitizeRelativePath(
            jsonDirObj.relativeFilePath(jsonInfo.absoluteFilePath()));
        ++jsonRelativePathCounts[QDir::cleanPath(relativePath).toLower()];
    }

    if (QMessageBox::warning(
        this,
        tr("Confirm Batch Import"),
        tr("Recursively import %1 JSON file(s) into matching W3D/WLT files?\n\n"
           "Only exact relative-path matches will be used. Every successful target "
           "will first be serialized and validated in a temporary file, backed up "
           "as .bak, and then replaced atomically. Ambiguous matches and JSON "
           "imports with warnings will be skipped.")
            .arg(jsonFiles.size()),
        QMessageBox::Yes | QMessageBox::Cancel,
    ...
    QStringList warningSkips;
    QStringList failures;
    bool canceled = false;

    for (int i = 0; i < jsonFiles.size(); ++i) {
        const QFileInfo& jsonInfo = jsonFiles.at(i);
        const QString jsonRelativePath = SanitizeRelativePath(
            jsonDirObj.relativeFilePath(jsonInfo.absoluteFilePath()));
        const QString jsonRelativeKey = QDir::cleanPath(jsonRelativePath).toLower();
        progress.setValue(i);
        progress.setLabelText(tr("Importing %1 (%2/%3)")
            .arg(jsonRelativePath)
            .arg(i + 1)
            .arg(jsonFiles.size()));
        QCoreApplication::processEvents();
        if (progress.wasCanceled()) {
        ...
        if (jsonRelativePathCounts[jsonRelativeKey] > 1) {
            ambiguous << tr("%1 (duplicate JSON relative path)").arg(jsonRelativePath);
            continue;
        }

        const auto targetIt = w3dByJsonRelativePath.find(jsonRelativeKey);
        if (targetIt == w3dByJsonRelativePath.end()) {
            unmatched << jsonRelativePath;
            continue;
        }
        if (targetIt->second.size() != 1) {
            ambiguous << tr("%1 (%2 matching W3D/WLT files)")
                .arg(jsonRelativePath)
                .arg(targetIt->second.size());
            continue;
        }
        const QString w3dPath = targetIt->second.constFirst();

        QFile jsonFile(jsonInfo.absoluteFilePath());
        if (!jsonFile.open(QIODevice::ReadOnly)) {
            failures << tr("%1 (cannot read JSON: %2)")
                .arg(jsonRelativePath, jsonFile.errorString());
            continue;
        }
        const QByteArray jsonBytes = jsonFile.readAll();
        if (jsonFile.error() != QFileDevice::NoError) {
            failures << tr("%1 (failed while reading JSON: %2)")
                .arg(jsonRelativePath, jsonFile.errorString());
            continue;
        }
        jsonFile.close();
        ...
        try {
            ...
            failures << tr("%1 (invalid JSON: %2)")
                .arg(jsonRelativePath, QString::fromUtf8(e.what()));
            continue;
        }
        ...
        try {
            if (!cd.fromJson(doc, &importWarnings)) {
                failures << tr("%1 (fromJson returned false)").arg(jsonRelativePath);
                continue;
            }
        }
        catch (const std::exception& e) {
        ...
        if (cd.getChunks().empty()) {
            failures << tr("%1 (JSON contains no chunks)").arg(jsonRelativePath);
            continue;
        }
        if (!importWarnings.empty()) {
            QStringList warningPreview;
            const std::size_t previewCount = std::min<std::size_t>(importWarnings.size(), 3);
            warningPreview.reserve(static_cast<int>(previewCount));
            for (std::size_t warningIndex = 0; warningIndex < previewCount; ++warningIndex) {
                warningPreview << QString::fromStdString(importWarnings[warningIndex]);
            }
            if (importWarnings.size() > previewCount) {
                warningPreview << tr("... (%1 additional warnings)")
                    .arg(static_cast<int>(importWarnings.size() - previewCount));
            }
            warningSkips << tr("%1 (%2 warning(s); target unchanged): %3")
                .arg(jsonRelativePath)
        ...
        QTemporaryFile stagedFile(
            QDir(QFileInfo(w3dPath).absolutePath())
                .absoluteFilePath(QStringLiteral(".ow3d-import-XXXXXX.tmp")));
        stagedFile.setAutoRemove(true);
        if (!stagedFile.open()) {
            failures << tr("%1 (failed to create staged file: %2)")
                .arg(jsonRelativePath, stagedFile.errorString());
            continue;
        }
        const QString stagedPath = stagedFile.fileName();
        stagedFile.close();

        if (!cd.saveToFile(stagedPath.toStdString())) {
            failures << tr("%1 (serialization failed; target unchanged)")
                .arg(jsonRelativePath);
            continue;
        }

        ChunkData stagedValidation;
        if (!stagedValidation.loadFromFile(stagedPath.toStdString())
            || stagedValidation.getChunks().empty())
        {
            failures << tr("%1 (staged W3D/WLT failed validation; target unchanged)")
                .arg(jsonRelativePath);
            continue;
        }

        if (!createBackupFile(w3dPath)) {
            failures << tr("%1 (failed to create backup; target unchanged)")
                .arg(jsonRelativePath);
            continue;
        }

        QString replaceError;
        if (!ReplaceFileAtomically(stagedPath, w3dPath, replaceError)) {
            failures << tr("%1 (atomic replacement failed; target unchanged: %2)")
                .arg(jsonRelativePath, replaceError);
            continue;
        }
    ...
    QString summary = tr("%1\nImported %2 of %3 JSON file(s) into %4.")
        ...
        .arg(w3dDir);
    if (successCount > 0) {
        summary += tr("\nEach replaced target has a .bak backup.");
    }
    ...
    if (!ambiguous.isEmpty()) {
        QStringList preview = ambiguous.mid(0, 20);
        if (ambiguous.size() > preview.size()) {
            preview << tr("... (%1 additional ambiguous paths)")
                .arg(ambiguous.size() - preview.size());
        }
        summary += tr("\n\nAmbiguous paths left unchanged (%1):\n%2")
            .arg(ambiguous.size())
            .arg(preview.join("\n"));
    }

    if (!warningSkips.isEmpty()) {
        QStringList preview = warningSkips.mid(0, 20);
        if (warningSkips.size() > preview.size()) {
            preview << tr("... (%1 additional warning skips)")
                .arg(warningSkips.size() - preview.size());
        }
        summary += tr("\n\nImports with warnings left unchanged (%1):\n%2")
            .arg(warningSkips.size())
            .arg(preview.join("\n"));
    }
...
void MainWindow::on_actionValidateRoundTripBatch_triggered()
{
    ...
    executeRoundTripValidation(srcDir, outDir, selectedRunMode, true);
}

int MainWindow::runCommandLineRoundTripValidation(
    const QString& sourceDirectory,
    const QString& outputDirectory,
    const QString& modeToken)
{
    const QString normalizedMode = modeToken.trimmed().toUpper();
    ValidatorRunMode runMode = ValidatorRunMode::Both;
    if (normalizedMode == QStringLiteral("STRUCTURED")
        || normalizedMode == QStringLiteral("STRUCTURED_PREFERRED"))
    {
        runMode = ValidatorRunMode::StructuredPreferred;
    }
    else if (normalizedMode == QStringLiteral("HEX")
        || normalizedMode == QStringLiteral("HEX_ONLY"))
    {
        runMode = ValidatorRunMode::HexOnly;
    }
    else if (normalizedMode != QStringLiteral("BOTH")) {
        std::cerr << "Invalid --verify-mode value. Use both, structured, or hex.\n";
        return 2;
    }

    return executeRoundTripValidation(
...
int MainWindow::executeRoundTripValidation(
    const QString& srcDir,
    const QString& outDir,
    ValidatorRunMode selectedRunMode,
    bool interactive)
{
    if (!QFileInfo(srcDir).isDir()) {
        const QString message = tr("Source directory does not exist: %1").arg(srcDir);
        if (interactive) {
            QMessageBox::warning(this, tr("Round-Trip Validate"), message);
        }
        else {
    ...
    if (!QDir().mkpath(outDir)) {
        const QString message = tr("Cannot create output directory: %1").arg(outDir);
        if (interactive) {
            QMessageBox::warning(this, tr("Round-Trip Validate"), message);
        }
        else {
    ...
    if (!QDir().mkpath(runDirPath) || !QDir().mkpath(failuresRootPath) || !QDir().mkpath(workRootPath)) {
        const QString message = tr("Failed to create output folders under %1").arg(outDir);
        if (interactive) {
            QMessageBox::warning(this, tr("Round-Trip Validate"), message);
        }
        else {
    ...
    if (!reportFile.open(QIODevice::WriteOnly | QIODevice::Truncate | QIODevice::Text)) {
        const QString message = tr("Cannot write report file: %1").arg(reportPath);
        if (interactive) {
            QMessageBox::warning(this, tr("Round-Trip Validate"), message);
        }
        else {
    ...
    progress.setWindowModality(Qt::WindowModal);
    progress.setMinimumDuration(interactive ? 0 : std::numeric_limits<int>::max());
    progress.setAutoClose(false);
    progress.setAutoReset(false);
    progress.setValue(0);
    ...
    if (!interactive) {
        std::cout << "Validating " << discoveredFileCount << " inputs across "
            << modesToRun.size() << " serialization mode(s).\n";
    }
    ...
    for (int i = 0; i < discoveredFileCount; ++i) {
        ...
        for (const JsonSerializationMode mode : modesToRun) {
            ...
            else if (runCounter > 0 && (runCounter % 100) == 0) {
                std::cout << "Processed " << runCounter << "/" << totalRuns << " runs.\n";
            }
            if (interactive && progress.wasCanceled()) {
                canceled = true;
                break;
            }
    ...
    if (interactive) {
        ...
        std::cout << summary.toStdString() << '\n';
        std::cout << "ROUNDTRIP_REPORT=" << QDir::toNativeSeparators(reportPath).toStdString() << '\n';
    }

    if (canceled) {
        return 130;
    }
    return (failCount > 0 || !discoveryWarnings.isEmpty()) ? 1 : 0;
}
